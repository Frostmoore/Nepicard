<x-app-layout>
    <div x-data="qrBurner()" x-init="init()" class="max-w-xl mx-auto mt-10 p-6 bg-gray-800 rounded shadow text-white">
        <h1 class="text-2xl font-semibold mb-6">Riscatta Tessera</h1>

        <div class="mb-6">
            <button type="button" @click="openScanner()" 
                    class="w-full bg-red-600 hover:bg-red-500 text-white font-semibold py-3 px-4 rounded transition">
                <i class="fa-solid fa-fire text-lg mr-2"></i> Scannerizza Tessera
            </button>
        </div>
        ...
        <template x-if="foundCode">
            <div class="space-y-3 bg-gray-700 p-4 rounded">
                ...
                <template x-if="!foundCode.user_id">
                    <div>
                        <span class="font-semibold text-gray-300">Saldo punti:</span>
                        <span x-text="foundCode.points"></span>
                    </div>
                </template>

                <template x-if="foundCode.user_id">
                    <div class="text-yellow-400 text-sm">Questa tessera è associata a un utente, non può essere riscattata.</div>
                </template>

                <form x-show="foundCode.status === 'active' && !foundCode.user_id" method="POST" action="{{ route('business.burn.store') }}">
                    @csrf
                    <input type="hidden" name="code" :value="foundCode.code">
                    <button type="submit" class="mt-4 w-full bg-red-600 hover:bg-red-500 text-white font-semibold py-3 px-4 rounded">
                        <i class="fa-solid fa-check mr-2"></i> Conferma Riscatto Tessera
                    </button>
                </form>
            </div>
        </template>
